...
const CardPage = () => {
  ...
  return (
    <main class="uk-container uk-container-large" uk-height-viewport="expand: true">
        <h1 class="uk-heading-divider">Ваши мероприятия</h1>
        <Suspense fallback={<div uk-spinner="ratio: 3" class="uk-position-center"></div>}>
            <Await resolve={UserEvents}>
                { 
                    (EventsMas) => (
                    <div uk-filter=".js-filter">
                        <ul class="uk-subnav uk-subnav-pill">
                            <li uk-filter-control=".tag-next">
                                <a href="#" class="my-event-next">
                                    Будущие
                                    <span class="uk-badge">
                                        {EventsMas.filter(x => x.tag == "tag-next").length}
                                    </span>
                                </a>
                            </li>
                            <li uk-filter-control=".tag-prev">
                                <a href="#" class="my-event-next">
                                    Прошедшие
                                    <span class="uk-badge">
                                        {EventsMas.filter(x => x.tag == "tag-prev").length}
                                    </span>
                                </a>
                            </li>
                        </ul>
                        {isMobile1 ? (
                            <div id="my-event-block" uk-slider="sets: true" class="uk-slider uk-slider-container">
                                <div class="uk-position-relative uk-margin-small-top uk-visible-toggle">
                                    <ul id="my-event-block-list" class="uk-slider-items uk-child-width-1-2@s uk-child-width-1-3@m uk-child-width-1-4@l uk-grid js-filter">
                                        {eventsForUser(UserEvents)}
                                    </ul>
...
const eventsForUserLoader = async () => {
    return defer({
        UserEvents: getEventsForUser()
    }) 

}

function eventsForUser(UserEvents)  { return(
    <Suspense>
        <Await resolve={UserEvents}>
            {
                (EventsMas) => (<>
...
export {CardPage, eventsForUserLoader};